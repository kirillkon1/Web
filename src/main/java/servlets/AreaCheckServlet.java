...
public class AreaCheckServlet extends HttpServlet {
	...
	private boolean getResult(double x, double y, double r) {
		return inFirstQuad(x, y, r) || inSecondQuad(x, y, r) || inThirdQuad(x, y, r);
	}

	// Quadrants of coordinate system

	private boolean inFirstQuad(double x, double y, double r){
		return (x >= 0 && y >= 0 && x <= r / 2 && y <= r && x / 2 + y <= r);
	}

	private boolean inSecondQuad(double x, double y, double r){
		return (x <= 0 && y >= 0 && y <= r && x >= -r);
	}

	private boolean inThirdQuad(double x, double y, double r){
